...
MclabOdom::MclabOdom(ros::NodeHandle nh) : nh(nh)
{
  ...
  imu_sub = nh.subscribe(imu_topic, 1, &MclabOdom::imuCallback, this);
  dvl_sub = nh.subscribe(dvl_topic, 1, &MclabOdom::dvlCallback, this);
  mocap_sub = nh.subscribe(mocap_topic, 1, &MclabOdom::mocapCallback, this);
  ...
  ROS_INFO("MclabOdom initialized");
}
...
void MclabOdom::imuCallback(const sensor_msgs::Imu& imu_msg)
{
...
void MclabOdom::dvlCallback(const geometry_msgs::TwistWithCovarianceStamped& twist_msg)
{
...
void MclabOdom::mocapCallback(const geometry_msgs::PoseStamped& msg)
{
